...
class AIClient:
    ...
    async def _make_api_request(self, prompt: str) -> Dict[str, Any]:
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are an expert translator specializing in Chinese → Shath'Yar translation. "
                            "\n\n【Shath'Yar语言定义】\n"
                            "Shath'Yar是魔兽世界中上古之神(Old Gods)及其仆从(n'raqi、aqir)的语言。'Shath'Yar'在其自己的语言中也指代上古之神本身。\n"
                            "这是虚空领主创造的宇宙恐怖语言，具有神秘、古老、邪恶的特性，用于表达虚空力量和恐怖概念。\n"
                            "该语言在游戏中被暗影牧师虚空形态和虚空武器使用者说出。\n\n"
                            "【翻译权威性原则】\n"
                            "1. 官方词典条目具有绝对权威性，是第一信源，必须严格遵循\n"
                            "2. 专有名词(如人名、地名、神名)必须精确使用官方译法，绝不可创新\n"
                            "3. 用户生成的参考信息仅供辅助，绝不能与官方权威冲突\n\n"
                            "【输出要求】\n"
                            "- 输出自然的Shath'Yar语言文本，非代码或结构化数据\n"
                            "- 保持原文标点和语句边界的对应关系\n"
                            "- 避免机械的逐字替换，要有语法变化和结构调整\n"
                            "- 对于词典中存在的词汇或句子，必须复用精确的拼写以确保一致性\n"
                            "- 只输出Shath'Yar翻译结果，不要任何解释或分析"
                        )
                    },
                    {
                        "role": "user", 
        ...
        log_data = {
            ...
            'headers': headers,
            'payload': payload,
            'prompt_extracted': payload.get('messages', [{}])[-1].get('content', '') if payload.get('messages') else ''
        }
        
        log_file = Path(__file__).parent / "real_ai_requests.json"
        try:
            if log_file.exists():
    ...
    def _build_chinese_to_shathyar_prompt(self, chinese_text: str,
        ...
        """Build AI prompt for Chinese to Shathyar translation with proper authority hierarchy"""
        
        ctx = dictionary_context or {}
        
        # 🔍 专有名词检测 - 在原文中识别可能的专有名词
        import re
        # 检测可能的专有名词模式：包含外文字符、人名特征等
        proper_noun_patterns = [
            r'[A-Za-z\']+',  # 英文字符（如N'Zoth）
            r'恩佐斯|克苏恩|亚煞极|尤格萨隆|奥杜尔|艾泽拉斯|诺森德|潘达利亚',  # 已知的魔兽专有名词
        ]
        
        detected_proper_nouns = []
        for pattern in proper_noun_patterns:
            matches = re.findall(pattern, chinese_text)
            detected_proper_nouns.extend(matches)
        
        # 📚 第一优先级：官方词典条目 (最高权威性，第一信源)
        relevant_entries = ctx.get("relevant_entries", [])[:15]  # 增加官方词典条目数量
        official_dict_examples = []
        proper_noun_mappings = []  # 专门记录专有名词映射
        
        for entry in relevant_entries:
            cn = entry.get('origin_cn', '').strip()
            sh = entry.get('shathyar', '').strip()
            if cn and sh:
                official_dict_examples.append(f"  {cn} → {sh}")
                # 如果是专有名词，特别标记
                if any(noun in cn for noun in detected_proper_nouns) or any(char.isupper() for char in sh):
                    proper_noun_mappings.append(f"  【专有名词】{cn} → {sh}")
        
        # 🔒 第二优先级：强制词汇映射表 (官方权威映射)
        ...
        if glossary:
            for g in glossary:
                ...
                if cn and sh:
                    ...
                    # 如果是专有名词，也要特别标记
                    if any(noun in cn for noun in detected_proper_nouns):
                        proper_noun_mappings.append(f"  【专有名词】{cn} → {sh}")
        
        # 🔍 第三优先级：RAG用户翻译参考 (辅助参考，不得干扰权威性)
        rag_context = ctx.get("rag_context", {})
        similar_translations = rag_context.get("similar_translations", [])
        user_rag_references = []
        if similar_translations:
            for i, st in enumerate(similar_translations[:2], 1):  # 减少用户数据影响
                user_mark = "✓用户确认" if st["is_user_confirmed"] else "AI生成"
                user_rag_references.append(f"  {st['source_text']} → {st['translated_text']} ({user_mark})")
        
        # 🏗️ 构建权威性明确的prompt结构
        prompt_parts = []
        
        # === 官方权威信息部分（绝对优先） ===
        prompt_parts.append("【官方权威翻译规范 - 绝对优先遵循】")
        
        # 如果检测到专有名词，优先展示专有名词映射
        if proper_noun_mappings:
            prompt_parts.append("🏛️ 【关键专有名词官方权威映射 - 必须100%精确使用】：")
            for mapping in proper_noun_mappings[:5]:  # 限制数量但确保显示
                prompt_parts.append(mapping)
            prompt_parts.append("")
        
        if official_dict_examples:
            prompt_parts.append("📚 官方词典权威条目（第一信源，必须严格参考）：")
            prompt_parts.extend(official_dict_examples)
            prompt_parts.append("")
        
        if mandatory_mappings:
            prompt_parts.append("🔒 强制词汇映射（官方权威，必须精确使用）：")
            prompt_parts.extend(mandatory_mappings)
            prompt_parts.append("")
        
        # === 基础示例部分 ===
        prompt_parts.append("【官方Shathyar语言基础示例】")
        ...
        # === 用户辅助参考（仅供参考，不得干扰权威性） ===
        if user_rag_references:
            prompt_parts.append("【用户翻译参考 - 仅作辅助参考，不得干扰官方权威】")
            prompt_parts.extend(user_rag_references)
            prompt_parts.append("")
        
        # === 翻译指令部分 ===
        prompt_parts.append("【翻译任务】")
        prompt_parts.append(f"请将以下中文翻译为Shath'Yar语：{chinese_text}")
        if detected_proper_nouns:
            prompt_parts.append(f"⚠️ 检测到专有名词：{', '.join(detected_proper_nouns[:3])} - 必须使用官方译法")
        prompt_parts.append("")
        
        # === 权威性约束说明 ===
        constraints = [
            "⚠️ 绝对权威要求（按优先级严格执行）：",
            "1. 【最高优先级】如输入包含官方词典条目，必须严格使用官方词典的准确翻译",
            "   - 特别注意：专有名词(人名、地名、神名、物品名)必须100%精确使用官方译法",
            "   - 例如：恩佐斯 = N'Zoth，克苏恩 = C'Thun，亚煞极 = Y'Shaarj",
            "2. 【第二优先级】如输入包含强制词汇映射，必须精确使用对应的Shath'Yar写法",
            "   - 强制映射具有不可违背的约束力",
            "3. 【参考优先级】可适度参考用户翻译的风格，但绝不能与上述官方权威冲突",
            "4. 严格按照官方Shath'Yar语言规范，保持神秘、古老、邪恶的语言风格",
            "5. 专有名词检测要求：",
            "   - 识别中文专有名词(大写字母、人名、地名特征)",
            "   - 优先查找官方词典中的精确对应",
            "   - 绝不允许对官方专有名词进行创造性翻译",
            "6. 只输出Shath'Yar翻译结果，不要任何解释或分析",
            "",
            "🎯 翻译："
        ]
